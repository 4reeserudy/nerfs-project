...
def _query_model_chunks(
    ...
    rgb   = torch.cat(rgb_outs, dim=0).reshape(B, n, 3)   # (B,n,3)
    sigma = torch.cat(sig_outs, dim=0).reshape(B, n, 1)   # (B,n,1)
    return rgb, sigma
...
def forward_batch(
    ...
    # 4) rendering terms (careful unpack & shapes)
    _tmid_from_edges, deltas = compute_deltas(t_edges=t_edges)            # (B,n),(B,n)
    # (We keep t_mid from sampling; _tmid_from_edges ≈ t_mid if no perturb.)
    alpha = alpha_from_sigma(sigma, deltas)                               # (B,n)
    T     = transmittance_from_alpha(alpha)                               # (B,n)
    w     = weights_from_alpha_T(alpha, T)                                # (B,n)

    # 5) background color as (B,3)
    ...
    # 6) composite foreground (no white-bg add here)
    rgb_fg, acc = composite_rgb(rgb_s, w, white_bg=False)                 # (B,3),(B,)

    # 7) mix with arbitrary bg: rgb = fg + (1-acc)*bg
    rgb = rgb_fg + (1.0 - acc).unsqueeze(-1) * bg
